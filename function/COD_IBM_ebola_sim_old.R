...
ebola_network_sim <- function(
  ...
  layer1_hh,                      # household edges (from, to)
  layer2_comm,                    # community edges (from, to, from_cell, to_cell)
  layer3_hcw_edges,               # HCW-HCW edges (from, to, hospital_id)
  layer3_admission,               # admission lookup (person_id, hh_id, hospital_id, hcw_list)
  cell_dist,                      # cell distance object (dist_mat + cell_lookup)
  ...
  # ── Build neighbor lookup from edge lists ──────────────────────────────────
  # Index: person row index (1..N) → list(household, community, hcw)
  message("Building neighbor lookup tables...")
  N <- nrow(nodes)

  # person_id → row index mapping
  pid_to_idx <- integer(max(nodes$person_id))
  pid_to_idx[nodes$person_id] <- seq_len(N)

  # Layer 1: household neighbors
  hh_nbrs <- vector("list", N)
  for (k in seq_len(nrow(layer1_hh))) {
    i <- pid_to_idx[layer1_hh$from[k]]
    j <- pid_to_idx[layer1_hh$to[k]]
    hh_nbrs[[i]] <- c(hh_nbrs[[i]], j)
    hh_nbrs[[j]] <- c(hh_nbrs[[j]], i)
  }

  # Layer 2: community neighbors
  comm_nbrs <- vector("list", N)
  for (k in seq_len(nrow(layer2_comm))) {
    i <- pid_to_idx[layer2_comm$from[k]]
    j <- pid_to_idx[layer2_comm$to[k]]
    comm_nbrs[[i]] <- c(comm_nbrs[[i]], j)
    comm_nbrs[[j]] <- c(comm_nbrs[[j]], i)
  }

  # Layer 3a: HCW-HCW neighbors
  hcw_nbrs <- vector("list", N)
  for (k in seq_len(nrow(layer3_hcw_edges))) {
    i <- pid_to_idx[layer3_hcw_edges$from[k]]
    j <- pid_to_idx[layer3_hcw_edges$to[k]]
    hcw_nbrs[[i]] <- c(hcw_nbrs[[i]], j)
    hcw_nbrs[[j]] <- c(hcw_nbrs[[j]], i)
  }

  # Layer 3b: admission lookup — row index → HCW row indices
  admission_nbrs <- vector("list", N)
  for (k in seq_len(nrow(layer3_admission))) {
    i <- pid_to_idx[layer3_admission$person_id[k]]
    hcw_row_ids <- pid_to_idx[layer3_admission$hcw_list[[k]]]
    admission_nbrs[[i]] <- hcw_row_ids
  }

  message(sprintf("  Neighbor lookup built for %d individuals", N))
  ...
  while (length(active_queue) > 0) {
    ...
    phase1_contacts <- list(
      list(nbrs = hh_nbrs[[idx]],   p_inf = p_inf_household,  ctype = 1L,
