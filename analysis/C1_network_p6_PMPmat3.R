...
# now generalized to THREE unique-partner models (Bernoulli s=1 / Linear /
# Linear-half-rate), and produce visualization figures for each.
#
# v4 changes (this revision):
#   - Removed the Bernoulli s=0.5 variant entirely. Only s=1 (the original
#     model) remains as the Bernoulli option.
#   - Added a new "linear_half" model: same as "linear" (weekly/monthly
#     grow with no saturation, daily keeps its Bernoulli factor), but the
#     weekly/monthly growth RATE is halved — i.e. only half of events are
#     treated as genuinely new/unique people, the other half assumed to be
#     repeat contacts. This replaces s=0.5 as the "more conservative than
#     full-linear" alternative hypothesis.
...
#       "s1"          : Bernoulli, T_eff = T_avg (the ORIGINAL v2 model —
#                       kept as the default/backward-compatible
#                       close_unique_3wk / physical_unique_3wk used by p7)
#       "linear"      : daily keeps its Bernoulli factor (1/21); weekly and
#                       monthly assume every event is a brand-new person
#                       (appear_factor = 1, unique == events for those two)
#       "linear_half" : same as "linear", but weekly/monthly's growth rate
#                       is halved (appear_factor = 0.5) — i.e. only half of
#                       weekly/monthly events are treated as genuinely new
#                       people, the rest as repeat contacts
...
library(dplyr)
library(cowplot)
library(terra)   # only used in Step 11 (population-weighted curve diagnostics)
...
#   linear      : daily = Bernoulli(s=1); weekly/monthly = events (rate=1,
#                 "every event is a brand-new person")
#   linear_half : same as linear, but weekly/monthly's growth rate is
#                 halved (rate=0.5, "only half of events are new people")
# ------------------------------------------------------------
model_defs <- list(
  s1          = list(type = "bernoulli", s = 1.0),
  linear      = list(type = "linear", rate = 1.0),
  linear_half = list(type = "linear", rate = 0.5)
)
model_labels <- c(
  s1          = "Bernoulli",
  linear      = "Linear",
  linear_half = "Linear, half rate"
)

bernoulli_factor_single <- function(T_avg_days, s = 1) {
  T_eff <- max(T_avg_days * s, 1)
  p     <- 1 / T_eff
...
compute_appear_factor <- function(model) {
  if (model$type == "linear") {
    # "Fully independent / brand-new partner each event" is a genuine
    # alternative hypothesis for weekly/monthly (it's ambiguous whether
    # those partners are a persistent roster or an ever-rotating cast).
    # It does NOT make sense for daily: "daily" contact is definitionally
    # a repeated relationship with the same person (T_avg=1 day is about
    # as unambiguous as POLYMOD's labels get), so daily always keeps its
    # Bernoulli-derived factor here too. Since T_avg=1 already forces
    # p=1 in the Bernoulli formula, this factor is fixed at 1/21
    # regardless of model — it just needs to NOT be overridden.
    # weekly/monthly get model$rate directly (1 for "linear", 0.5 for
    # "linear_half") — appear_factor for the linear-type models IS the
    # rate, since unique(L) = (E/L)*L*rate = E*rate.
    return(c(
      daily   = unname(bernoulli_factor_single(avg_revisit_days["daily"])),
      weekly  = model$rate,
      monthly = model$rate
    ))
  }
  sapply(avg_revisit_days, function(T_avg) bernoulli_factor_single(T_avg, model$s))
}
...
saveRDS(
  list(
    ...
    # ALL THREE models' unique matrices (s1 / linear / linear_half)
...
# ============================================================
# Step 11: Cumulative unique-edge curves over time, by location,
#          for all three models — reuses model_defs / avg_revisit_days /
#          window_L / close_events_3wk already computed above (no reload
#          from .rds, no re-derivation of model logic).
#
# This is the ONLY place in the whole script that touches population
# (WorldPop) — needed here because collapsing the 16 age groups into a
# single representative scalar per location requires knowing how common
# each age group actually is. The per-capita matrices built above stay
# population-free throughout.
#
# Location-specific age weighting:
#   work   -> ages 20+  (adults only; nobody under 20 is meaningfully "at work")
#   school -> ages 5-19 (school-age population only)
#   other  -> full population (everyone can plausibly have "other" contacts)
#
# Per-stratum curve rule (same logic as compute_appear_factor() above,
# generalized from a single time point t=L to a continuous t):
#   - Bernoulli model (s1): every stratum uses the Bernoulli-with-scale
#     formula with s=1.
#   - Linear models (linear, linear_half): daily ALWAYS uses the
#     Bernoulli(s=1) curve (a "brand-new person every day" contradicts what
#     "daily" contact even means); only weekly/monthly use the linear
#     (fully-independent) curve, scaled by the model's rate (1.0 for
#     "linear", 0.5 for "linear_half").
# ============================================================
...
message("\nLoading WorldPop cod_t_ files (Step 11 only)...")
...
message("\n== Population-relevant age subset per location (Step 11) ==")
for (loc in split_locations) {
...
# ------------------------------------------------------------
# Step 11a: representative E per stratum x location (location-relevant
#           population weighting), reusing close_events_3wk from Step 3
# ------------------------------------------------------------
E_rep_pop <- sapply(split_locations, function(loc) {
  sapply(names(freq_strata), function(st) {
    pop_weighted_mean_loc(rowSums(close_events_3wk[[st]][[loc]]), loc)
  })
})
rownames(E_rep_pop) <- names(freq_strata)
colnames(E_rep_pop) <- split_locations

E_total_pop  <- colSums(E_rep_pop)
E_weekly_pop <- E_rep_pop["weekly", ]

message("\n== E_rep_pop (location-relevant pop-weighted 21-day events) ==")
print(round(E_rep_pop, 3))

# ------------------------------------------------------------
# Step 11b: curve functions — SAME model_defs as the matrix construction
#           above (Step 0b), generalized from a single t=L evaluation to
#           a continuous t sequence.
# ------------------------------------------------------------
bernoulli_unique_curve <- function(E, T_avg_days, s, L, t_seq) {
  T_eff <- max(T_avg_days * s, 1)
  p_eff <- 1 / T_eff
  N_eff <- E * T_eff / L
...
linear_unique_curve <- function(E, L, t_seq, rate = 1) (E / L) * t_seq * rate

stratum_curve <- function(E, stratum_name, model, t_seq) {
  if (model$type == "bernoulli") {
    return(bernoulli_unique_curve(E, avg_revisit_days[stratum_name], model$s,
                                  window_L, t_seq))
  }
  # linear-type models: daily always keeps Bernoulli(s=1); weekly/monthly
  # go linear, scaled by the model's rate (1.0 for "linear", 0.5 for
  # "linear_half")
  if (stratum_name == "daily") {
    return(bernoulli_unique_curve(E, avg_revisit_days["daily"], 1.0,
                                  window_L, t_seq))
  }
  linear_unique_curve(E, window_L, t_seq, rate = model$rate)
}

combined_curve <- function(E_by_stratum, loc, model, t_seq) {
  Reduce(`+`, lapply(names(freq_strata), function(st)
    stratum_curve(E_by_stratum[st, loc], st, model, t_seq)))
}

t_max_curve <- 28
t_seq <- seq(0, t_max_curve, by = 0.25)

model_curve_colors <- c(s1 = "#0F6E56", linear = "#B5651D", linear_half = "#D4A017")

# ------------------------------------------------------------
# Step 11c: sanity checks at t = 21 (curve should match the saved
#           close_unique_3wk_by_model matrices from Step 7)
# ------------------------------------------------------------
message("\n== Sanity check @t=21: combined s1 curve vs close_unique_3wk_by_model$s1 (summed) ==")
for (loc in split_locations) {
  at21  <- combined_curve(E_rep_pop, loc, model_defs$s1, window_L)
  saved <- sum(sapply(names(freq_strata), function(st)
    pop_weighted_mean_loc(rowSums(close_unique_3wk_by_model$s1[[st]][[loc]]), loc)))
  message(sprintf("  %-6s: curve@21=%.4f | saved=%.4f | diff=%.2e",
                  loc, at21, saved, abs(at21 - saved)))
}

# ------------------------------------------------------------
# Step 11d: shared panel builder + renderer
# ------------------------------------------------------------
make_curve_panel <- function(loc, llab, df_builder, y_label,
                             show_y = FALSE, show_legend = FALSE) {
  df <- df_builder(loc)
  df$model <- factor(df$model, levels = names(model_defs))

  ggplot(df, aes(x = day, y = unique, color = model)) +
    geom_line(linewidth = 0.9) +
    geom_vline(xintercept = window_L, linetype = "dashed",
               color = "grey40", linewidth = 0.5) +
    scale_color_manual(values = model_curve_colors,
                       breaks = names(model_defs),
                       labels = model_labels[names(model_defs)],
                       name   = "Model") +
    scale_x_continuous(breaks = seq(0, t_max_curve, by = 7)) +
    labs(
      title = llab,
      x     = "Days",
...
render_curve_grid <- function(df_builder, y_label, out_path, width = 12, height = 3.8) {
  panels <- lapply(seq_along(split_locations), function(i)
    make_curve_panel(split_locations[i], plot_loc_labels_d[i], df_builder, y_label,
                     show_y = (i == 1)))
  legend_shared <- get_legend(
    make_curve_panel(split_locations[1], plot_loc_labels_d[1], df_builder, y_label,
                     show_legend = TRUE)
  )
  grid_main <- plot_grid(plotlist = panels, nrow = 1, ncol = 3)
  grid_full <- plot_grid(grid_main, legend_shared, nrow = 1, rel_widths = c(1, 0.32))
  ggsave(out_path, grid_full, width = width, height = height, dpi = 300)
  message(sprintf("Saved: %s", out_path))
}

# ------------------------------------------------------------
# Figure: daily+weekly+monthly COMBINED, all 3 models, by location
# ------------------------------------------------------------
build_df_total_curve <- function(loc) {
  do.call(rbind, lapply(names(model_defs), function(mn) {
    data.frame(day = t_seq,
               unique = combined_curve(E_rep_pop, loc, model_defs[[mn]], t_seq),
               model  = mn)
  }))
}

render_curve_grid(
  build_df_total_curve,
  y_label  = "Cumulative unique edges\n(daily+weekly+monthly combined,\nlocation-relevant pop-weighted avg individual)",
  out_path = "figure/MPMmat/DRC_total_scale_comparison_1x3.png"
)

# ------------------------------------------------------------
# Figure: WEEKLY-only breakdown, all 3 models, by location
# ------------------------------------------------------------
build_df_weekly_curve <- function(loc) {
  do.call(rbind, lapply(names(model_defs), function(mn) {
    data.frame(day = t_seq,
               unique = stratum_curve(E_weekly_pop[loc], "weekly", model_defs[[mn]], t_seq),
               model  = mn)
  }))
}

render_curve_grid(
  build_df_weekly_curve,
  y_label  = "Cumulative unique weekly edges\n(location-relevant pop-weighted avg individual)",
  out_path = "figure/MPMmat/DRC_weekly_scale_comparison_1x3.png"
)

# ------------------------------------------------------------
# Figure: Bernoulli s=1 (current model) ONLY, split by stratum —
#   daily/weekly/monthly all drawn in the SAME green color
#   (model_curve_colors["s1"]), distinguished only by line type:
#     Daily     -> dotted
#     Weekly    -> solid
#     Monthly+  -> dashed
#   Shown over 63 days (9 weeks) so monthly's slow saturation is visible,
#   with the 21-day observation window marked by a vertical dashed line.
# ------------------------------------------------------------
t_max_stratum <- 63
t_seq_stratum <- seq(0, t_max_stratum, by = 0.5)

linetype_map_stratum <- c("Daily" = "dotted", "Weekly" = "solid", "Monthly+" = "dashed")

build_df_stratum_s1 <- function(loc) {
  do.call(rbind, lapply(names(freq_strata), function(st) {
    data.frame(
      day     = t_seq_stratum,
      unique  = stratum_curve(E_rep_pop[st, loc], st, model_defs$s1, t_seq_stratum),
      stratum = unname(stratum_display[st])
    )
  }))
}

make_stratum_panel_s1 <- function(loc, llab, show_y = FALSE, show_legend = FALSE) {
  df <- build_df_stratum_s1(loc)
  df$stratum <- factor(df$stratum, levels = c("Daily", "Weekly", "Monthly+"))

  ggplot(df, aes(x = day, y = unique, linetype = stratum)) +
    geom_line(color = model_curve_colors["s1"], linewidth = 0.9) +
    geom_vline(xintercept = window_L, linetype = "dashed",
               color = "grey60", linewidth = 0.4) +
    scale_linetype_manual(values = linetype_map_stratum, name = "Frequency") +
    scale_x_continuous(breaks = seq(0, t_max_stratum, by = 21)) +
    labs(
      title = llab,
      x     = "Days",
      y     = if (show_y) "Cumulative unique edges\n(pop-weighted avg individual, s=1)" else NULL
    ) +
    theme_minimal(base_size = 9) +
    theme(
      plot.title       = element_text(size = 10, face = "bold", hjust = 0.5),
...
panels_stratum_s1 <- lapply(seq_along(split_locations), function(i)
  make_stratum_panel_s1(split_locations[i], plot_loc_labels_d[i], show_y = (i == 1)))

legend_stratum_s1 <- get_legend(
  make_stratum_panel_s1(split_locations[1], plot_loc_labels_d[1], show_legend = TRUE)
)

grid_stratum_s1      <- plot_grid(plotlist = panels_stratum_s1, nrow = 1, ncol = 3)
grid_stratum_s1_full <- plot_grid(grid_stratum_s1, legend_stratum_s1,
                                  nrow = 1, rel_widths = c(1, 0.22))

ggsave("figure/MPMmat/DRC_bernoulli_by_stratum_1x3.png",
       grid_stratum_s1_full, width = 12, height = 3.6, dpi = 300)
message("Saved: figure/MPMmat/DRC_bernoulli_by_stratum_1x3.png")
