...
# Purpose:
#   Validate synthetic population output from p4 for all 3 simulation cases:
#     1. Map of household density per cell
#     2. Map of mean household size per cell
#     3. Map of mean age per cell
#     4. Household size distribution
#     5. Age distribution (Prem 16 groups)
#     6. Random household viewer
#     7. Cell-level household composition
#     8. 100 sample households composition plot
# ==============================================================================
...
# ==============================================================================
# [Helper] Run all validation for one case
# ==============================================================================

validate_case <- function(tag, synpop_dir, fig_dir) {
  synpop_file <- file.path(synpop_dir,
                           paste0(tag, "_synthetic_population.rds"))
  ...
  p1 <- ggplot(cell_summary, aes(x=x, y=y, fill=log1p(n_hh))) +
    geom_raster() +
    scale_fill_viridis_c(option="magma", name="log(HH+1)",
  ...
  p2 <- ggplot(cell_summary, aes(x=x, y=y, fill=mean_hh_size)) +
    ...
    coord_fixed() +
    labs(title="Mean household size per cell", x=NULL, y=NULL) +
    theme_void() +
    theme(legend.position="right",
          plot.title=element_text(size=11, face="bold"))
  ...
  p3 <- ggplot(cell_summary, aes(x=x, y=y, fill=mean_age)) +
    ...
    coord_fixed() +
    labs(title="Mean age per cell", x=NULL, y=NULL) +
    theme_void() +
    theme(legend.position="right",
          plot.title=element_text(size=11, face="bold"))
  ...
  # ── [6] Random household viewer ───────────────────────────────
  set.seed(42)
  sample_hh_ids <- sample(unique(pop_full$hh_id),
                          min(15L, n_distinct(pop_full$hh_id)))
  ...
  for (hid in sample_hh_ids) {
    ...
    cat(sprintf("  HH %-6s | cell (%7.4f, %7.4f) | size=%d | ages: %s\n",
                hid, cell_xy$x[1], cell_xy$y[1],
                nrow(members),
                paste(sort(members$age), collapse=", ")))
  }

  # ── [7] Cell-level household composition (5 sample cells) ─────
  ...
  # ── [8] 100 sample households composition plot ────────────────
  set.seed(123)
  s_ids <- sample(unique(pop_full$hh_id), min(100, n_distinct(pop_full$hh_id)))
  s_dat <- pop_full %>%
    filter(hh_id %in% s_ids) %>%
    group_by(hh_id) %>%
    arrange(age, .by_group=TRUE) %>%
    mutate(member_idx=row_number()) %>%
    ungroup() %>%
    mutate(hh_id    = factor(hh_id, levels=s_ids),
           age_group = cut(age,
                           breaks=c(0,18,65,Inf),
                           labels=c("0-17 (Child)","18-64 (Adult)","65+ (Elderly)"),
                           right=FALSE))

  p_100 <- ggplot(s_dat, aes(x=member_idx, y=age, fill=age_group)) +
    geom_col(width=0.8) +
    ...
    scale_fill_manual(values=c("0-17 (Child)"  = "#66c2a5",
                               ...
                               "65+ (Elderly)" = "#8da0cb"),
                      name="Age group") +
    labs(title    = sprintf("%s — 100 Sample households", tag),
         subtitle = "Each panel = one household | bars = individuals by age",
         x=NULL, y="Age") +
    theme_minimal() +
  ...
  ggsave(file.path(fig_dir, sprintf("%s_100_hh_sample.png", tag)),
         plot=p_100, width=12, height=12, dpi=150)
...
# ==============================================================================
# [Main] Run validation for all 3 cases
# ==============================================================================

for (tag in case_tags) {
  validate_case(tag, synpop_dir, fig_dir)
}
