...
bucket_weights <- c(
  kernel_integral(0,     0.5),   # Bucket 0: same cell
...
pop_files <- pop_files_all[grepl("case1_1M|case2_Ituri|case3_Kivu",
                                 basename(pop_files_all))]
message(sprintf("  Case files: %d", length(pop_files)))
...
for (fi in seq_along(pop_files)) {
  ...
  # First try territory name match, then fall back to spatial boundary
  terr_hospitals <- hf_joined %>%
    filter(paste0(gsub("[^A-Za-z0-9]", "_", NAME_1), "_",
                  gsub("[^A-Za-z0-9]", "_", NAME_2)) == tag)

  if (nrow(terr_hospitals) == 0) {
    # Fallback: hospitals within case boundary (coordinate-based)
  ...
  # Pass all persons to C++ at once — no chunk overhead
  N        <- nrow(pop_nodes)
  t_layer2 <- proc.time()[["elapsed"]]

  comm_edges <- build_edges_cpp(
    ...
    seed       = network_seed + fi * 999L
  )

  elapsed_l2 <- round(proc.time()[["elapsed"]] - t_layer2, 1)
  comm_edges <- as.data.frame(comm_edges)
  comm_edges <- comm_edges[!duplicated(comm_edges[c("from","to")]), ]
  message(sprintf("    Layer 2: %d edges (%.1f sec, deduped)", nrow(comm_edges), elapsed_l2))
  saveRDS(comm_edges, out_layer2)
  rm(comm_edges, cdist); gc()
