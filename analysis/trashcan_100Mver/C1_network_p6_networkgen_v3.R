...
build_cell_dist <- function(cell_tbl, radius_m, radius_km) {
  message("test -1")
  ...
  message("test -2")
  # Build upper-triangle vectors directly from frNN output
  from_l <- vector("list", n_cells)
  to_l   <- vector("list", n_cells)
  dist_l <- vector("list", n_cells)
  message("test -3")
  for (ci in seq_len(n_cells)) {
    # if (ci %% 10000L == 0L) message(sprintf("    cell_dist: %d / %d cells", ci, n_cells))
    nbrs  <- nn_id[[ci]]
    dists <- nn_dist[[ci]] / 1000  # meters → km
    keep  <- nbrs > ci             # Upper triangle only
    if (!any(keep)) next
  ...
  message("test -4")
  from_c <- unlist(from_l, use.names = FALSE)
  to_c   <- unlist(to_l,   use.names = FALSE)
  dist_c <- unlist(dist_l, use.names = FALSE)
  ...
  if (length(from_c) == 0) {
    return(list(
      ...
      nn_cell_id = nn_id,
      cell_mx    = cell_mx, cell_my = cell_my,
      from_vec   = integer(0), to_vec = integer(0), dist_vec = numeric(0)
    ))
  }
  ...
  list(
    ...
    nn_cell_id = nn_id,
    cell_mx    = cell_mx, cell_my = cell_my,
    from_vec   = from_c, to_vec = to_c, dist_vec = dist_c
  )
}
...
for (fi in seq_along(pop_files)) {
  ...
  cdist <- build_cell_dist(cell_tbl, radius_m, radius_km)
