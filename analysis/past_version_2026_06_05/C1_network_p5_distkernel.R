...
# Plot 3: CDF of mixture exponential
cdf_curves_df <- map_dfr(idx, function(i) {
  pp <- post$p[i]
  a1 <- post$a1[i]
  a2 <- post$a2[i]
  tibble(
    distance = dist_seq,
    # CDF of mixture: p * F1(d) + (1-p) * F2(d)
    cdf = pp * (1 - exp(-a1 * dist_seq)) +
      (1 - pp) * (1 - exp(-a2 * dist_seq))
  )
})

cdf_summary_df <- cdf_curves_df %>%
  group_by(distance) %>%
  summarise(
    median = median(cdf),
    lo     = quantile(cdf, 0.025),
    hi     = quantile(cdf, 0.975),
    .groups = "drop"
  )

# Observed cumulative proportions at bin boundaries (0, 10, 100, inf)
obs_cum_df <- tibble(
  distance = c(0, 10, 100, 200),
  cum_obs  = c(0, obs[1], obs[1] + obs[2], 1)
)

p3_plot <- ggplot() +
  geom_ribbon(data = cdf_summary_df,
              aes(x = distance, ymin = lo, ymax = hi),
              fill = "#185FA5", alpha = 0.3) +
  geom_line(data = cdf_summary_df,
            aes(x = distance, y = median),
            color = "#185FA5", linewidth = 1) +
  # Observed cumulative proportions as reference points
  geom_step(data = obs_cum_df,
            aes(x = distance, y = cum_obs),
            color = "#D85A30", linewidth = 0.8, linetype = "dashed") +
  geom_point(data = obs_cum_df,
             aes(x = distance, y = cum_obs),
             color = "#D85A30", size = 2) +
  scale_x_continuous(
    trans  = "log10",
...
ggsave(file.path("figure/C1_p5_kernel/cummkernel.png"),
       p3_plot, width = 8, height = 3, dpi = 500)
