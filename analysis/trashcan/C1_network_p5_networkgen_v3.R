...
# Single-core, chunk-based community edge builder
# Chunks of 10k individuals processed and flushed to disk immediately
# → accumulation lists stay small throughout, no GC slowdown
create_comm_edges_chunked <- function(node_df, cell_mat, cell_sizes,
                                      nn_cell_id, prem_mu, size = 0.1,
                                      chunk_size = 10000L, seed = 42) {
  set.seed(seed)
  N        <- nrow(node_df)
  ...
  chunk_results <- vector("list", n_chunks)  # Accumulate results in memory
  ...
  for (ci in seq_len(n_chunks)) {
    ...
    # Accumulation lists sized to chunk — stays small, no GC pressure
    from_l      <- vector("list", n_chunk)
    to_l        <- vector("list", n_chunk)
    from_cell_l <- vector("list", n_chunk)
    to_cell_l   <- vector("list", n_chunk)
    k <- 0L
    ...
    for (i in chunk_ids) {
      ...
      # O(1) candidate lookup via cell membership matrix
      nbr_cells <- nn_cell_id[[cell_ids[i]]]
      if (length(nbr_cells) == 0L) next

      cands <- unlist(lapply(nbr_cells, function(cj) {
        sz <- cell_sizes[cj]
        if (sz == 0L) return(NULL)
        cell_mat[cj, seq_len(sz)]
      }), use.names = FALSE)
    ...
    # Store chunk result directly in memory — no disk I/O
    if (k > 0L) {
      chunk_results[[ci]] <- data.frame(
    ...
    message(sprintf("  [Layer 2] chunk %d / %d | %d / %d (%.1f%%) | %.0f indiv/sec | ETA %.1f min",
                    ci, n_chunks, idx_end, N,
                    100 * idx_end / N, rate, eta))

    # Free chunk lists immediately
    rm(from_l, to_l, from_cell_l, to_cell_l);  gc()
  }

  # Combine all chunk results at once
  message("  Combining chunks...")
  out <- bind_rows(chunk_results)
  rm(chunk_results); gc()

  if (nrow(out) == 0) return(data.frame(from      = integer(0),
                                        to        = integer(0),
                                        from_cell = integer(0),
                                        to_cell   = integer(0)))
