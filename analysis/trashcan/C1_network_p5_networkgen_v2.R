...
# cell_members: list indexed by cell_id → person indices in pop_nodes
# Used by workers to expand cell neighbours into individual candidates
message("  Building cell membership index...")
cell_members <- split(seq_len(nrow(pop_nodes)), pop_nodes$cell_id)

create_comm_edges_parallel <- function(node_df, cell_members, nn_cell_id,
                                       prem_mu, size = 0.1,
                                       n_cores = 15L, seed = 42) {
  N        <- nrow(node_df)
  ...
  worker <- function(chunk_ids, hh_ids, cell_ids, age_grps,
                     cell_members, nn_cell_id, prem_mu, size, seed) {
    set.seed(seed)

    # Use lists for accumulation — avoid repeated c() memory reallocation
    from_l      <- vector("list", length(chunk_ids))
    to_l        <- vector("list", length(chunk_ids))
    from_cell_l <- vector("list", length(chunk_ids))
    to_cell_l   <- vector("list", length(chunk_ids))
    k <- 0L
    ...
    for (i in chunk_ids) {
      ...
      # Expand neighbour cells → individual candidates
      # Cell frNN already enforces 10km radius — no haversine filter needed
      nbr_cells <- nn_cell_id[[cell_ids[i]]]
      if (length(nbr_cells) == 0) next

      cands <- unlist(cell_members[nbr_cells], use.names = FALSE)
      if (length(cands) == 0) next

      # Exclude self and same household
      cands <- cands[cands != i & hh_ids[cands] != hh_ids[i]]
      if (length(cands) == 0) next
      ...
      if (any(keep)) {
        ...
        from_l[[k]]      <- rep(i,           sum(keep))
        to_l[[k]]        <- sampled_keep
        from_cell_l[[k]] <- rep(cell_ids[i], sum(keep))
        to_cell_l[[k]]   <- cell_ids[sampled_keep]
      }
    }
    if (k == 0L) return(NULL)
    ...
    # Collapse lists in one shot — much faster than repeated c()
    data.frame(
      from      = unlist(from_l[seq_len(k)],      use.names = FALSE),
      to        = unlist(to_l[seq_len(k)],        use.names = FALSE),
      from_cell = unlist(from_cell_l[seq_len(k)], use.names = FALSE),
      to_cell   = unlist(to_cell_l[seq_len(k)],   use.names = FALSE),
      stringsAsFactors = FALSE
    )
  }
  ...
  clusterExport(cl,
                varlist = c("hh_ids", "cell_ids", "age_grps",
                            "cell_members", "nn_cell_id",
                            "prem_mu", "size"),
                envir = environment())
  chunk_args <- lapply(seq_along(chunks), function(ci)
    list(chunk_ids = chunks[[ci]], seed = seed + ci))

  # pblapply: parallel execution with progress bar
  results <- pblapply(chunk_args, function(args)
    worker(args$chunk_ids, hh_ids, cell_ids, age_grps,
           cell_members, nn_cell_id, prem_mu, size, args$seed),
    cl = cl)

  out <- bind_rows(Filter(Negate(is.null), results))
  # Integer hash deduplication — faster than paste(from, to)
  N_max <- max(out$from, out$to)
  out[!duplicated(out$from * (N_max + 1L) + out$to), ]
}

t0         <- proc.time()
comm_edges <- create_comm_edges_parallel(
  pop_nodes, cell_members, nn_cell_id, prem_comm_mu,
  size    = comm_nb_size,
  n_cores = n_cores,
  seed    = network_seed
)
...
message(sprintf("  Saved: %s (%.1f MB)", basename(out_path),
                file.info(out_path)$size / 1e6))
rm(comm_edges, nn_cell_id, cell_members); gc()
